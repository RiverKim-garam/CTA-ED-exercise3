...
We will be using a subset of these data, taken from EU speeches given by four politicians. These are provided by the authors at <https://dataverse.harvard.edu/dataset.xhtml?persistentId=doi:10.7910/DVN/S4IZ8K>.
...
##1. Compute distance measures such as "euclidean" or "manhattan" for the MP tweets as above, comparing between tweets by MPs and tweets by PM, Theresa May. \# Euclidean distance
...
euclidean_dis_mat <- as.matrix(euclidean_dis) 
```

```{r}
## tidy format

# select 23rd row of the matrix
eucl_dis_mat <- as.data.frame(euclidean_dis_mat[23, c(1:22, 24)])

#rename colume
colnames(eucl_dis_mat) <- "dist_may"

#rownames
eucl_dis_mat <- tibble::rownames_to_column(eucl_dis_mat, "username")
```
...
# Euclidean distance score shows the distance between tweets of Theresa May and MPs. Euclidean distance and other similarity scores above do not seem to have in common directly in that tweets of Penny Mordaunt and Brandon Lewis are far from PM's depite their medium similarities.
```
...
```{r}
#Use *speeches* data with other textstat_readability scores

#coleman
speeches$coleman <- textstat_readability(speeches$text, measure = "Coleman", remove_hyphens = TRUE, min_sentence_length = 1, max_sentence_length = 10000)

#meanSentenceLength
speeches$sent_length <- textstat_readability(speeches$text, measure = "meanSentenceLength", remove_hyphens = TRUE, min_sentence_length = 1, max_sentence_length = 10000)

#SMOG
speeches$smog <- textstat_readability(speeches$text, measure = "SMOG", remove_hyphens = TRUE, min_sentence_length = 1, max_sentence_length = 10000)

#Scrabble
speeches$scrabble <- textstat_readability(speeches$text, measure = "Scrabble", remove_hyphens = TRUE, min_sentence_length = 1, max_sentence_length = 10000)

#remain the scores
speeches$coleman <- speeches$coleman$Coleman
speeches$sent_length <- speeches$sent_length$meanSentenceLength
speeches$smog <- speeches$smog$SMOG
speeches$scrabble <- speeches$scrabble$Scrabble

```

```{r}
#get mean of scores and N of speeches for each speaker
sum_corpus_score <- speeches %>%
  group_by(speaker) %>%
  summarise(mean_cole = mean(coleman, na.rm = TRUE),
            SD_cole = sd(coleman, na.rm = TRUE), 
            mean_length = mean(sent_length, na.rm = TRUE),
            SD_length = sd(sent_length, na.rm = TRUE), 
            mean_smog = mean(smog, na.rm = TRUE),
            SD_smog = sd(smog, na.rm = TRUE), 
            mean_scrabble = mean(scrabble, na.rm = TRUE),
            SD_scrabble = sd(scrabble, na.rm = TRUE), 
            N=length(speaker))
...
```{r}
# graph
# First I make four different graphs for each measure
ggplot(sum_corpus_score, aes(x=speaker, y=mean_cole)) +
  geom_bar(stat = "identity") +
  xlab("") +
  ylab("Mean Coleman")

ggplot(sum_corpus_score, aes(x=speaker, y=mean_length)) +
  geom_bar(stat = "identity") +
  xlab("") +
  ylab("Mean sentence length")

ggplot(sum_corpus_score, aes(x=speaker, y=mean_smog)) +
  geom_bar(stat = "identity") +
  xlab("") +
  ylab("Mean smog")

ggplot(sum_corpus_score, aes(x=speaker, y=mean_scrabble)) +
  geom_bar(stat = "identity") +
  xlab("") +
  ylab("Mean scrabble")


# combine the graph with multiple y values
sum_corpus_score_long <- sum_corpus_score %>%
  gather(key = "variable", value = "value", mean_cole, mean_length, mean_smog, mean_scrabble)

ggplot(sum_corpus_score_long, aes(x = speaker, y = value, fill = variable)) +
...
```{r}
# there is no date data in 'speeches'
